...
int coinChange_r(int coins[], int moneyNeeded, int n)
{
    ...
    if (coins[n - 1] <= moneyNeeded)
        return coinChange_r(coins, moneyNeeded - coins[n - 1], n-1) + coinChange_r(coins, moneyNeeded, n - 1);
    else
...
int coinChange_m(int coins[], int moneyNeeded, int n)
{
    ...
    if (n == 0)
        ...
        return dp[moneyNeeded][n] = coinChange_m(coins, moneyNeeded - coins[n - 1], n-1) + coinChange_m(coins, moneyNeeded, n - 1);
    else
    ...
    for (int i = 0; i <= moneyNeeded; i++)
        for (int j = 0; j <= n; j++)
        {
            if (i == 0)
                ...
                dp[i][j] = dp[i - coins[j - 1]][j-1] + dp[i][j - 1];
            else
                dp[i][j] = dp[i][j - 1];
        }
    return dp[moneyNeeded][n];
